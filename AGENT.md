...
- `supabase/functions/ai-career-coach/bsc-knowledge.test.ts` — the actual test suite for `isBroadStartingAsk()`, plus a sync-check for `TOPIC_CATEGORIES` against `KNOWLEDGE_BASE`. Extend this file when changing that routing logic; don't just eyeball it or run one-off `node -e` scripts.
...
- **Two-layer routing, not one — but layer 1 is a flag, not a verdict:**
  1. `isBroadStartingAsk()` in `bsc-knowledge.ts` — a deterministic, phrasing-independent regex check run before the AI call. If a message mentions tech/career/job/coding/programming and names no specific skill/role/challenge, it's *flagged* as a probable broad ask. This exists because relying on the AI router alone to reliably notice a broad ask was demonstrably unreliable (`gpt-5-nano` doesn't follow "always ask first" instructions consistently).
  2. The flag does **not** skip the AI call or force a fixed answer — the keyword regex can't see context (e.g. it matches "tech" in "a field that is *not* related to tech" just as readily as in "help me get into tech"). Flagged messages go through the *same* routing call as everything else, with an added instruction (`BROAD_ASK_HINT` in `index.ts`) telling the AI to default to `inviteUserContext` but override that default if the message actually means something else. `inviteUserContext`'s tool schema requires `question`/`options` from the model itself, so this also produces a properly tailored clarifying question instead of a generic fixed one — still no extra AI call, no extra latency, since it rides along on the routing call that already happens for every message.
