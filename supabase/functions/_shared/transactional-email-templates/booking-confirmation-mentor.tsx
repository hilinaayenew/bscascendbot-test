...
const BookingConfirmationMentorEmail = ({ mentorName, bookerName, date, time, timezone, meetingLink }: Props) => {
  const firstName = mentorName?.split(' ')[0] || 'there'
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>New session booked — {bookerName || 'Someone'}, {date || ''} at {time || ''}</Preview>
      <Body style={main}>
        <Container style={container}>
          <Heading style={h1}>New session booked</Heading>
          <Text style={text}>Hi {firstName},</Text>
          <Text style={text}>You have a new session booked on Ascendency. Here are the details:</Text>
          <Text style={detailRow}><strong>With:</strong> {bookerName || 'A community member'}</Text>
          ...
          <Text style={text}>
            You can view and manage your upcoming sessions from your Sessions page on the platform.
          </Text>
          <Text style={text}>
...
export const template = {
  component: BookingConfirmationMentorEmail,
  subject: (data: Record<string, any>) =>
    `New session booked — ${data.bookerName || 'Someone'}, ${data.date || ''} at ${data.time || ''}`,
  displayName: 'Booking confirmation (mentor)',
  previewData: { mentorName: 'Sarah Mensah', bookerName: 'Amara Osei', date: 'Monday, January 15, 2025', time: '10:00 AM', timezone: 'Africa/Accra', meetingLink: 'https://meet.google.com/abc-defg-hij' },
