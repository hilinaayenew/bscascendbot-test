...
const BookingReminderMentorEmail = ({ mentorName, bookerName, date, time, timezone, meetingLink }: Props) => {
  const firstName = mentorName?.split(' ')[0] || 'there'
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Reminder: session tomorrow with {bookerName || 'your mentee'} at {time || ''}</Preview>
      <Body style={main}>
...
export const template = {
  component: BookingReminderMentorEmail,
  subject: (data: Record<string, any>) =>
    `Reminder: session tomorrow with ${data.bookerName || 'your mentee'} at ${data.time || ''}`,
  displayName: 'Booking reminder (mentor)',
  previewData: { mentorName: 'Sarah Mensah', bookerName: 'Amara Osei', date: 'Tuesday, January 16, 2025', time: '10:00 AM', timezone: 'Africa/Accra', meetingLink: 'https://meet.google.com/abc-defg-hij' },
