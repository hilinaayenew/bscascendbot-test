...
class BoteMindset extends WordaliseFunction {
  ...
  async generateResponse(prompt: string, _question: string): Promise<string> {
    ...
    const messages: OAIMessage[] = [
      {
        role: "system",
        content: BOTEMA_VALUES + "\n\n" + BOTEMA_SYSTEM_PROMPT + ` Be warm and honest when addressing a mindset challenge, but never open with a stock acknowledgment phrase like "I hear you," "That sounds hard," or "Great question." Get straight into a helpful, specific response. ${STAND_WITH_HER} ${NEVER_OFFER_TO_ACT} ${PLAIN_LANGUAGE} ${LONG_FORM_ESCAPE_HATCH}`,
      },
      ...history,
      { role: "user", content: prompt },
    ];
