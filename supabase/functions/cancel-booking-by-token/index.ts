...
// Public endpoint: cancel a booking using its cancel_token. No auth required —
// possession of the unguessable token is the proof of authorisation. Sends
// cancellation emails to both parties using server-resolved addresses.
Deno.serve(async (req) => {
  ...
  if (!cancelToken || typeof cancelToken !== 'string') {
    return new Response(JSON.stringify({ error: 'cancelToken required' }), {
      status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  }
